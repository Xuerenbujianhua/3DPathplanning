...
%地图规模在Makemap3D函数中设置
map = Makemap3D_obstacle;%获取三维栅格地图
source=[10 10 1]; %起点
%200  420 10
goal=[200 420 10];%终点
max_item = 2000;%最大迭代次数
...
popNum = 1; %蚁群数量
tic
%[aco_path,aco_cost] = aco(source,goal,map,popNum);%蚁群主函数
[aco_path,aco_cost,aco_Number_of_searches,aco_Number_of_successful_searches]= aco(source,goal,map,popNum);
aco_time = toc;
